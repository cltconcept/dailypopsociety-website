...
- 🔒 Sécurité : image Docker **non-root** (`USER node`, uid 1000) sur le **port 8080** (< 1024 interdit à un non-root ; ⚠️ `ports_exposes` à corriger sur l'app Coolify), `chown -R node:node /app /data` avant le `VOLUME`, `HEALTHCHECK` sur `/health`, et fichiers de tests exclus de l'image
...
- Coolify maquettes `http://46.224.83.139:8000` (⚠️ pas l'instance de production Noveo) : projet `dailypopsociety` (uuid `sx4bmqtu54ejgpf4p2mpxdwg`), application uuid `o129qzwy4inm5tsgfrmqrtw7`, serveur `zw8ck4ckcw08gg00g8wwkkso`, build pack `dockerfile`, branche `main`, **port 8080** (⚠️ `ports_exposes` à passer de 80 à 8080 sur l'app — l'image écoute désormais sur 8080), domaine `https://dailypopsociety.chris-ia.com` (wildcard `*.chris-ia.com`, HTTPS automatique).
