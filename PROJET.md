...
### 2026-09-08
- ✨ Ajout : fond manga du générique, animé au scrub (demande utilisateur : « ça manque d'effet dans le background, style manga, ombre de one piece ») — lignes de vitesse qui convergent vers le titre et éclatent à l'impact, ombres pirates en SVG inline (chapeau de paille, galion à l'horizon, crâne aux os croisés, vagues seigaiha en CSS), onomatopées ゴゴゴ verticales qui rampent, « ドン!! » d'impact et éclair blanc quand le nom se fige ; évocations seulement, aucune œuvre sous licence, aucune image à charger (police japonaise système). ⚠️ GSAP fait tourner un `<svg>` racine autour de son coin : la transformation est portée par un `div` enveloppant
- ♻️ Refactor : hero sans les deux bandes de badges (décision utilisateur : « pas terrible sauf dans la typo ») — le défilé des 28 logos ne vit plus que dans les lettres de « DAILY POP » (trois tuiles du montage, accélération, léger resserrement du nom), 56 vignettes de moins à charger ; START transforme le hero en borne PLEIN ÉCRAN par une transition GSAP (nom et pied du générique reculent, écran qui s'ouvre depuis une ligne façon tube cathodique sous un éclair blanc, cascade en-tête/HUD/écran/classement ; extinction inverse à la fermeture) ; écran de jeu à la hauteur du viewport, classement en bandeau sous l'écran
